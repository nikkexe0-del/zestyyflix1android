...
export function WatchScreen() {
  ...
  // Fetch poster via our own Vercel serverless proxy → returns base64
  // This bypasses CORS entirely since it's same-origin
  const fetchPosterBase64 = async (tmdbUrl: string): Promise<string | null> => {
    try {
      const res = await fetch(`/api/imgproxy?url=${encodeURIComponent(tmdbUrl)}`);
  ...
  const buildShareBlob = async (): Promise<Blob> => {
    ...
    if (posterUrl) {
      const dataUrl = await fetchPosterBase64(posterUrl);
      if (dataUrl) {
        const img = new Image();
